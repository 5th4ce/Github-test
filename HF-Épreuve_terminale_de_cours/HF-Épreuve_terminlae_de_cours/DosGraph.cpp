...
//-----------------------------------------------------
void dessinerTableau(int DimensionX, int DimensionY, int PosX, int PosY, int CelluleX, int CelluleY)
{
	//Dessiner un tableau quadrillé
	//Dessin de la base
	for (int x=0;x<DimensionX*CelluleX;x+=CelluleX)
		for (int y=0;y<DimensionY*CelluleY;y+=CelluleY)
			dessinerCadre(PosX+x,PosY+y,PosX+CelluleX+x,PosY+CelluleY+y);

	//Ameliorer le tableau
	for (int x=0;x<DimensionX-1; x++)
	{
		//Côté Haut
		setXY(PosX+CelluleX+x*CelluleX,PosY);
		cout << char(194);
		//Côté Bas
		setXY(PosX+CelluleX+x*CelluleX , PosY+DimensionY*CelluleY );
		cout << char(193);
	}
	for (int x=0;x<DimensionY-1; x++)
	{
		//Côté Gauche
		setXY(PosX,PosY+CelluleY+x*CelluleY);
		cout << char (195);
		//Côté Droit
		setXY(PosX+(DimensionX*CelluleX),PosY+CelluleY+x*CelluleY);
		cout << char (180);
		//Centre
		for (int y=0;y < DimensionX-1; y++)
		{
			setXY(PosX+CelluleX+y*CelluleX,PosY+CelluleY+x*CelluleY);
			cout << char(197);
		}
	}
}